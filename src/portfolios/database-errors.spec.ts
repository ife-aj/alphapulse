...
describe('toDatabaseHttpException (portfolios)', () => {
  describe('unique violations (23505) are disambiguated by operation context', () => {
    it('maps 23505 on create portfolio to a duplicate-name 409', () => {
      const e = toDatabaseHttpException(pgError('23505'), 'create-portfolio');
    ...
    it('maps 23505 on rename portfolio to a duplicate-name 409', () => {
      const e = toDatabaseHttpException(pgError('23505'), 'rename-portfolio');
      expect(e).toBeInstanceOf(ConflictException);
      expect(e.getStatus()).toBe(409);
    });

    it('maps 23505 on add holding to a duplicate-symbol 409', () => {
      const e = toDatabaseHttpException(pgError('23505'), 'add-holding');
      expect(e).toBeInstanceOf(ConflictException);
      expect(e.getStatus()).toBe(409);
      expect(e.getResponse()).toEqual(
        expect.objectContaining({
    ...
    it('never echoes the raw database message or constraint name', () => {
      const e = toDatabaseHttpException(
        pgError(
          '23505',
          'duplicate key value violates unique constraint "uq_portfolios_user_lower_name"',
          'Key (user_id, lower(btrim(name)))=(…, tech) already exists.',
        ),
        'create-portfolio',
      );
      const body = JSON.stringify(e.getResponse());
      expect(body).not.toContain('uq_portfolios_user_lower_name');
  ...
  it('maps FK (23503) and RLS (42501) violations to a neutral 404', () => {
    for (const code of ['23503', '42501']) {
      const e = toDatabaseHttpException(pgError(code), 'add-holding');
      expect(e).toBeInstanceOf(NotFoundException);
      expect(e.getStatus()).toBe(404);
      expect(e.getResponse()).toEqual(
        expect.objectContaining({ message: 'Portfolio not found.' }),
      );
    }
  });
  ...
  it('maps invalid-UUID (22P02) and CHECK (23514) violations to 400', () => {
    expect(
      toDatabaseHttpException(pgError('22P02'), 'delete-portfolio'),
    ).toBeInstanceOf(BadRequestException);
    expect(
      toDatabaseHttpException(pgError('23514'), 'add-holding'),
    ).toBeInstanceOf(BadRequestException);
  });
  ...
  it('maps an unknown 23505 on other operations to a 503', () => {
    const e = toDatabaseHttpException(pgError('23505'), 'delete-holding');
    expect(e).toBeInstanceOf(ServiceUnavailableException);
    expect(e.getStatus()).toBe(503);
  });

  it('maps unknown PostgREST codes to a 503', () => {
    const e = toDatabaseHttpException(pgError('XX000'), 'list-holdings');
    expect(e).toBeInstanceOf(ServiceUnavailableException);
    expect(e.getStatus()).toBe(503);
  });
  ...
  it('passes an HttpException through unchanged', () => {
    const original = new NotFoundException('Portfolio not found.');
    expect(toDatabaseHttpException(original, 'delete-portfolio')).toBe(original);
  });
});
